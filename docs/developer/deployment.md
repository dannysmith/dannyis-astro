...
- **Keep `stableIconSetTimestamp` in `astro.config.mjs`.** astro-icon stamps the current time into `virtual:astro-icon` for the icon set built from `src/icons/`, and every page reaches that module through `Icon`. Without the pin, every route's dependency hash changes on every build and nothing is ever reused — silently, with no error.
