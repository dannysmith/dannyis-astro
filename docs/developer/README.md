# Developer Documentation Index

## Tier 2: Core Guides

Read these first for daily development:

- [architecture-guide.md](./architecture-guide.md) - Essential patterns and overview (START HERE)
- [component-patterns.md](./component-patterns.md) - Component development patterns
- [design.md](./design.md) - Visual philosophy and CSS architecture

## Tier 3: Specialized References

Consult when working on specific features:

- [accessibility-and-performance.md](./accessibility-and-performance.md) - Accessible and performant code
- [content-system.md](./content-system.md) - Content collections, RSS, build-time generation
- [content-authoring.md](./content-authoring.md) - Creating and editing articles/notes
- [seo.md](./seo.md) - SEO utilities and patterns
- [code-quality.md](./code-quality.md) - Quality checks and standards
- [testing.md](./testing.md) - Testing strategy and patterns
