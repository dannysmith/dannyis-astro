...
**Current state:**
These components exist in `src/components/mdx/` and `src/components/mdx/typography/` but are not in the barrel export file.
...
**Recommended fix:**
Add these components to `src/components/mdx/index.ts`:
```typescript
...
**Recommended fix:**
Create a utility function in `src/utils/content.ts`:
```typescript
export function filterPublishableContent<T extends { data: { draft?: boolean; styleguide?: boolean } }>(
  entries: T[]
): T[] {
  if (import.meta.env.PROD) {
...
**Confusion:**
Developers might look for reading time in:
