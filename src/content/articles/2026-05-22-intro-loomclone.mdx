...
import { LCVid, BookmarkCard, Callout, IntroParagraph } from '@components/index.js'
...
| #   | Tool                | Why                                                                                                                                                                                                                                              |
| --- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | `highpass` at 80Hz  | Strips out sub-speech rumble (HVAC hum, desk vibration, fan resonance) so the rest of the chain doesn't waste cycles on it.                                                                                                                      |
| 2   | `arnndn`            | The main speech denoiser – an RNN-based filter using the `cb.rnnn` model from [richardpl/arnndn-models](https://github.com/richardpl/arnndn-models). The model is trained on close-mic speech with general background noise, which makes it a great match for screen recordings. |
| 3   | `afftdn`            | Cleans up the *stationary* noise (mic hiss, AC drone) that the RNN-based denoiser leaves alone. The noise-floor estimate is profiled per recording.                                                                                              |
| 4   | `agate`             | A noise gate that silences anything below -45 dBFS in non-speech sections. Stops the next two filters from accidentally amplifying any residual noise during pauses.                                                                             |
| 5   | `dynaudnorm`        | Levels speech volume locally – mostly to recover the drops when I lean back from the mic. Only ever sees gated regions so it can't amplify noise.                                                                                                |
| 6   | `loudnorm`          | EBU R128 loudness normalisation to -14 LUFS / -1.5 dBTP (the web standard for places like YouTube). Run twice: the first pass measures the post-processed signal, the second uses those numbers to hit the target precisely.                    |
