...
We'll go into details later, but here's my basic "101" Claude Code advice:
...
- **React** – because everything is react these days.
...
3. **Automated checks we can run**. We run tests, linters, formatters and friends periodically while we work so we don't get caught by the enforced gates. We can run these against only the code we're working on, and immediately address any problems.
