...
`<LCVid />` renders a self-hosted video from [v.danny.is](https://v.danny.is) with build-time metadata. Most authors won't use it directly — passing a `v.danny.is` URL to `<Embed>` auto-dispatches to it.

Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque euismod, nisi eu consectetur consectetur, nisl nisi consectetur nisi, euismod euismod nisi nisi euismod.

<LCVid src="lcdemo-part-2" />

Lorem ipsum dolor sit amet, consectetur adipiscing elit. The same video with the transcript visible:

<LCVid src="lcdemo-part-2" showTranscript />

Minimal — player only, no title, meta or description. For when the surrounding prose has already introduced the video:

<LCVid src="lcdemo-part-2" showTitle={false} showMeta={false} showOpenLink={false} showDescription={false} />

Via `<Embed>` — equivalent to the default rendering above:
