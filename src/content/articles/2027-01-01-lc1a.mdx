...
1. Part 1 - **Introducing my very own loom**
2. Part 2 – [How my personal Loom works – an overview](/writing/loomclone-part-2)
</Callout>

<IntroParagraph>
Over the past couple of months I've been building a personal video-sharing platform to replace  [Loom](https://loom.com/), partly so it works how I've always wanted Loom to work and partly so I have long-term control over my own videos. It's basic functionality is broadly equivilent to Loom minus the stuff I never use, plus some me-specific extras. Building what I'm currently calling *LoomClone* has been mad fun and I've learned a **ton** (this is my first proper native macOS app, among other things).
</IntroParagraph>
...
2. Record **longer, more polished videos** for tutorials, guides, presenations and the like. These often end up embedded in evergreen Notion/Google docs and tend to be widely linked to from all sorts of places. When I say "more polished" I don't mean the *hours-in-final-cut-pro* polish I might put into a marketing video. More like *preparing for an All Hands or training session on Zoom*: I'm gonna prep an outline & some slides, tidy up my desktop, make sure all my demos and other windows are in the right place, maybe do a quick run-through **and then record in one take**. My default approach to this with Loom has always been to line up any slides or apps I need in fulscreen spaces and swipe between them as I record, adding any important talking-head segments by recording seperate Looms and then stitching them together in Loom's web editor.
...
1. **I don't own the cannonical URLs for my videos.** I've got many hundreds of historical Loom videos, many of which are embedded in documents, wikis, knowledge bases and archived slack channels in organisations I've long-since left. Every one of them points to `loom.com/something`, so while I own my actual videos (I can download them), Loom controls what matters most for content on the web: *the cannonical URs*.
2. **I don't have much control over the viewer experience.** Loom controls how my videos are served to viewers: It's impossible for me to control things like RSS feeds, custom embeded players, SEO and text-based endpoints to the degree I want.
...
<Callout color="blue" emoji="📜" title="Core Principles">

Given the potential for scope creep here, I started by defining some core principles. These underpin almost all the product decisions I made.
...
6. **Simplicity**. The whole system is designed with one goal in mind: **allowing me to record, host, and share videos**. Every feature must be in service of doing those things *really well*.  Similarly, the codebase and technology stack should be as simple as possible.
...
It's helpful to think about LoomClone as four seperate things, each with a distinct purpose and set of user needs. While this isn't quite accurate when it comes to the underlying tech, it's the framing I'll be using in this series of articles.

1. **The macOS recording app**. A native macOS menubar app for recording videos with as little friction as possible. It's only job is helping me get set up for a recording quickly and then streaming it up to the server once I hit record while dealing properly with all the pausing and mode switching that happens during recording.

2. **The backend API**. A web API which receives recordings from the mac app and handles storage, background post-processing, backup, error recovery and the like.

3. **The admin app**. A web interface which lets me manage, organise and edit my online videos, optimised to speed up the workdlows I care most about personally.

4. **The viewer-facing surface**. Serves the vidoes URLs to viewers along with discovery-focussed stuff like SEO and RSS feeds. This is all the public-facing stuff.

The rest of this article covers the **recording app**'s UI and demo's the main features of the **viewer-facing surface** and **admin app**. We'll go into detail about *how* these work in subsequent articles – for now I just wanna explain *what* they all do right now.

## The macOS recording app

The menubar app is deliberately very simple. Clicking it opens a popover like this.

![lc-menubar-app-1.png](../../assets/articles/2026-07-06-lc-menubar-app-1.png)

| Control | Description |
| ------- | ----------- |
| Displpay selector | Select the monitor to use when showing the screen |
| Camera selector | Select the camera to use |
| Mic selector | Select the microphone to use |
| Preview | Shows a preview of how the recording will look (based on the *starting mode* selected below) so I can confirm the right sources are selected and are looking good |
| Sound meter | For the selected mic so I can check I have decent audio levels |
| White balance | Adjust the camera white balance. I often want to tweak this depending on the curent lighting conditions |
| Brightness | Adjust the camera brightness |
| Starting mode selector | Choose between screen-only, camera-only and picture-in-picture mode. This only affects the preview above and the mode in which the recording will **start**. I can toggle between these freely while recording.
...
![lc-menubar-app-2.png](../../assets/articles/2026-07-06-lc-menubar-app-2.png)
...
![lc-recording-overlay.png](../../assets/articles/2026-07-06-lc-recording-overlay.png)
...
When in a camera mode a draggable preview of the camera feed is shown as an overlay. This is circular when in screen-and-camera and rectangular when in camera-only mode.

![lc-recording-pip.png](../../assets/articles/2026-07-06-lc-recording-pip.png)
![lc-recording-camonly.png](../../assets/articles/2026-07-06-lc-recording-camonly.png)
...
Besides the settings window and a few warnings which pop up if something goes wring while recording, **this is the whole UI of the macOS app**. 
...
![lc-demo-publicpage-screenshot.png](../../assets/articles/2026-07-06-lc-demo-publicpage-screenshot.png)
...
While the page is intentionally simple, fully controlling it's underlying code means I've been able to ensure these URLs embed properly in Notion and Google docs, have appropriate rich preview data when shared, and – for public videos – are discoverable via appropriate SEO, RSS feeds and the like.

![lc-embedding-notion.png](../../assets/articles/2026-07-06-lc-embedding-notion.png)

![lc-whatsapp-share.png](../../assets/articles/2026-07-06-lc-whatsapp-share.png)
...
<Callout title="Visibility" type="blue">
...
![lc-tagpage-public-demo.png](../../assets/articles/2026-07-07-lc-tagpage-public-demo.png)

We'll dig into some of the more interesting features and subtleties of the public serfice in subsequent articles, but for now I wanna give you a quick tour of the admin app for all this.

## The admin app, a lighting tour
