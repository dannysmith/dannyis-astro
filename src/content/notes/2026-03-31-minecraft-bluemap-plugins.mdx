...
3. A cheap way to run a persistant shared creative world with Cam, and also to spin up and manage ephemeral minecraft servers for various reasons.
...
When the heatmap is on, a little HUD will show the the inhabited time of the chunk under the cursor.
...
Selected chunks can be exported as either JSON or [MCA-Selector](https://github.com/Querz/mcaselector) compatable CSV. I intentionally decided against building the actual chunk deletion.
...
- Suitable memory, disk usage, CPU limites etc for a "permanent"-tier world.
