...
import { Accordion, ButtonLink } from '@components/mdx';
...
<Accordion header="Plain Accordion" plain>
...
Default rendering — title, meta and description fetched from [v.danny.is](https://v.danny.is):

<LCVid src="lcdemo-part-2" />

Minimal — just the player:

<LCVid src="lcdemo-part-2" showTitle={false} showMeta={false} showOpenLink={false} showDescription={false} />

Via `<Embed>` — any `v.danny.is` URL auto-dispatches to `<LCVid>`:
