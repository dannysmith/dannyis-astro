...
Aside from the toolbox page itself, there are four parts to this: a new JSON-type content collection, a `ToolCard` component, a script which creates or updates the content collection and a GitHub Actions workflow to run the script.
...
1. The tool page's canonical URI on `betterat.work`, plus confirmation it's been properly synced & published by Super.
